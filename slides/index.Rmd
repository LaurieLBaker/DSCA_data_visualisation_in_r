...
.right-plot[
![](`r knitr::fig_chunk("exercise-1", "png")`)
]
...
.left-code[
```{r exercise-1-answer, fig.show="hide"}
gapminder %>%
  filter(continent == "Americas", 
         year %in% c(1952, 2007)) %>%
  mutate(year = as.factor(year)) %>%
  ggplot() +
    ...
    labs(x = "Life Expectancy", 
         y = "Country", 
         colour = "Year") +
  ggtitle("Life Expectancy in the Americas 1952 vs 2007") #<<
```

]

.right-plot[
![](`r knitr::fig_chunk("exercise-1-answer", "png")`)
]
...
What does fct_reorder and coord_flip do? Try removing them.
...
.right-plot[
![](`r knitr::fig_chunk("exercise-2", "png")`)
]
...
Try recreating the following plot by filling in the blanks below

.left-code[

```{r exercise-3, echo = FALSE, fig.show = "hide"}
...
```{r exercise-3-blanks, fig.show="hide", eval = FALSE}
...
.right-plot[
![](`r knitr::fig_chunk("exercise-3", "png")`)
]
...
```{r exercise-3-show, echo = TRUE, fig.show = "hide"}
...
What's the relationship between Population and Year by continent? 

```{r What is the relationship between population and year by continent solution, eval = TRUE, fig.show = "hide"}
...
```{r What is the relationship between population and year by continent solution plot, eval = TRUE, echo = FALSE}
...
How can I tell the continents apart?
...
```{r gapminder-americas-2-2-1, fig.show="hide"}
...
.right-plot[
![](`r knitr::fig_chunk("gapminder-americas-2-2-1", "png")`)
]
...
```{r gapminder-americas-2-2-2, fig.show="hide"}
...
.right-plot[
![](`r knitr::fig_chunk("gapminder-americas-2-2-2", "png")`)
]
...
```{r gapminder-americas-2-2-a, fig.show="hide"}
...
.right-plot[
![](`r knitr::fig_chunk("gapminder-americas-2-2-a", "png")`)
]

What if we want to see the bars side by side
---
